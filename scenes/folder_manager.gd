...
func update_controls() -> void: # Updating form controls depending how much user has filled it
	...
	generate_folder_button.disabled = not (main_scene.user_preferences.has_default_path and customer_name != "" and project_name != "" and production_type_option.selected != -1 and editor_name != "" and info_locked)
