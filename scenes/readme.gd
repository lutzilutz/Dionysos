...
func generate_team() -> String:
	...
	if main_scene.use_contact:
		s += add_line(2, "Téléphone : " + main_scene.user_preferences.get_editor_from_name(main_scene.editor_name).phone)
		s += add_line(2, "E-mail : " + main_scene.user_preferences.get_editor_from_name(main_scene.editor_name).email)
	return s
